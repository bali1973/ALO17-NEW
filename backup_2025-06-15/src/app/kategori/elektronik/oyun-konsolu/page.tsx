...
const subcategories = [
  { id: 'playstation', name: 'PlayStation', icon: <FaGamepad className="inline mr-2 text-blue-600" /> },
  { id: 'xbox', name: 'Xbox', icon: <FaGamepad className="inline mr-2 text-green-600" /> },
  { id: 'nintendo', name: 'Nintendo', icon: <FaGamepad className="inline mr-2 text-red-600" /> },
  { id: 'aksesuar', name: 'Aksesuar', icon: <FaHeadset className="inline mr-2 text-gray-600" /> },
]

export default function OyunKonsoluCategoryPage() {
  const [selectedSubcategory, setSelectedSubcategory] = useState<string | null>(null)
  const [priceRange, setPriceRange] = useState<string | null>(null)
  const [condition, setCondition] = useState<string | null>(null)

  // Oyun konsolu ilanlarını filtrele
  const consoleListings = listings.filter(listing => 
    listing.category === 'elektronik' && 
    listing.subcategory === 'oyun-konsolu'
  )
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900">Oyun Konsolu</h1>
        <p className="text-gray-600 mt-2">
          PlayStation, Xbox, Nintendo ve oyun konsolu aksesuarları
        </p>
      </div>
