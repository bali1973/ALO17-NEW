...
export default function SpecialFoodsCategoryPage() {
  const [selectedCuisine, setSelectedCuisine] = useState<string | null>(null)
  const [priceRange, setPriceRange] = useState<string | null>(null)
  const [location, setLocation] = useState<string | null>(null)
  const [features, setFeatures] = useState<string[]>([])

  // Özel yemek ilanlarını filtrele
  const specialFoodListings = listings.filter(listing => 
    listing.category === 'yemek-icecek' && 
    listing.subcategory === 'ozel-yemekler'
  )

  // Mutfak türlerini topla
  const cuisines = Array.from(new Set(specialFoodListings.map(listing => listing.brand)))

  // Filtreleme fonksiyonu
  const filteredListings = specialFoodListings.filter(listing => {
