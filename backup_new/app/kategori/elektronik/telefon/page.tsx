...
export default function PhoneCategoryPage() {
  ...
  const phoneListings = listings.filter(listing => 
    listing.category === 'elektronik' && 
    listing.subcategory === 'telefon'
  )

  // Markaları topla
  const brands = Array.from(new Set(phoneListings.map(listing => listing.brand)))

  // Filtreleme fonksiyonu
  const filteredListings = phoneListings.filter(listing => {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900">Telefonlar</h1>
        <p className="text-gray-600 mt-2">
          En yeni ve popüler telefon modellerini inceleyebilirsiniz.
        </p>
      </div>
