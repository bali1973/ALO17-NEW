...
export default function MenClothingCategoryPage() {
  const [selectedBrand, setSelectedBrand] = useState<string | null>(null)
  const [priceRange, setPriceRange] = useState<string | null>(null)
  const [condition, setCondition] = useState<string | null>(null)
  const [size, setSize] = useState<string | null>(null)

  // Erkek giyim ilanlarını filtrele
  const menClothingListings = listings.filter(listing => 
    listing.category === 'giyim' && 
    listing.subcategory === 'erkek-giyim'
  )

  // Markaları topla
  const brands = Array.from(new Set(menClothingListings.map(listing => listing.brand)))

  // Filtreleme fonksiyonu
  const filteredListings = menClothingListings.filter(listing => {
    if (selectedBrand && listing.brand !== selectedBrand) return false
    if (condition && listing.condition !== condition) return false
    if (size && !listing.features.includes(size)) return false
    if (priceRange) {
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        ...
        <p className="text-gray-600 mt-2">
          Erkek giyim ürünlerini inceleyebilir, kıyafet, ayakkabı ve aksesuar seçeneklerini keşfedebilirsiniz.
        </p>
      </div>
