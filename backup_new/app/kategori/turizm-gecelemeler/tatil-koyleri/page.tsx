...
export default function ResortsCategoryPage() {
  const [selectedType, setSelectedType] = useState<string | null>(null)
  const [priceRange, setPriceRange] = useState<string | null>(null)
  const [location, setLocation] = useState<string | null>(null)
  const [features, setFeatures] = useState<string[]>([])

  // Tatil köyü ilanlarını filtrele
  const resortListings = listings.filter(listing => 
    listing.category === 'turizm-gecelemeler' && 
    listing.subcategory === 'tatil-koyleri'
  )

  // Tatil köyü tiplerini topla
  const types = Array.from(new Set(resortListings.map(listing => listing.type)))

  // Filtreleme fonksiyonu
  const filteredListings = resortListings.filter(listing => {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900">Tatil Köyleri</h1>
        <p className="text-gray-600 mt-2">
          Her bütçeye uygun, eğlence dolu tatil köyleri.
        </p>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-8">
        ...
        <div className="w-full md:w-64 flex-shrink-0">
          <div className="bg-white rounded-lg shadow-sm p-4">
            ...
            {/* Tatil Köyü Tipi Filtresi */}
            <div className="mb-6">
              <h3 className="font-medium mb-2">Tatil Köyü Tipi</h3>
          ...
          <div className="bg-white rounded-lg shadow-sm p-4">
            <div className="flex justify-between items-center">
              <span className="text-gray-600">
                {filteredListings.length} tatil köyü bulundu
              </span>
              <select 
          ...
          {filteredListings.length === 0 && (
            <div className="text-center py-12">
              <h3 className="text-xl font-semibold text-gray-900 mb-2">
                Tatil Köyü Bulunamadı
              </h3>
              <p className="text-gray-600">
                Seçtiğiniz kriterlere uygun tatil köyü bulunamadı. Lütfen filtreleri değiştirerek tekrar deneyin.
              </p>
            </div>
          )}
        </div>
