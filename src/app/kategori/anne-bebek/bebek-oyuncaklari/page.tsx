...
export default function BebekOyuncaklariPage() {
  ...
  useEffect(() => {
    ...
    fetch('/api/listings')
      .then(res => res.json())
      .then(data => {
        setListings(data.filter((l: any) => l.category === 'anne-bebek' && l.subcategory === 'bebek-oyuncaklari'));
        setLoading(false);
      })
      .catch(() => {
  ...
  return (
    <div className="container mx-auto px-4 py-8 flex flex-col md:flex-row gap-8">
            ...
            {subcategories.map((cat) => (
              <li key={cat.slug}>
                <Link href={`/kategori/anne-bebek/${cat.slug}`}
                  className={`block px-3 py-2 rounded transition ${cat.slug === 'bebek-oyuncaklari' ? 'bg-blue-50 text-blue-700 font-bold border border-blue-200' : 'hover:bg-gray-100'}`}
                >
                  {cat.name}
                </Link>
              </li>
      ...
      <main className="flex-1">
        <h1 className="text-3xl font-bold text-gray-900 mb-2">Bebek Oyuncakları</h1>
        <p className="text-gray-600 mb-6">Anne & Bebek kategorisinde Bebek Oyuncakları alt kategorisi</p>
        {loading ? (
          <div>Yükleniyor...</div>
        ) : error ? (
