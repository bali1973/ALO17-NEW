...
export default function AnneBebekPage() {
  ...
  const mainCategory = categories.find(cat => cat.slug === 'anne-bebek');
  ...
  useEffect(() => {
    fetch('/api/listings')
      .then(res => res.json())
      .then(data => {
        setListings(data.filter((listing: Listing) => listing.category === 'anne-bebek'));
      });
  }, []);
  ...
  return (
    <CategoryLayout
      ...
      activeSlug=""
      categoryBasePath="kategori/anne-bebek"
      city={city}
    ...
    >
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Anne & Bebek</h1>
        <p className="text-gray-600 mt-2">
          Bebek giyim, oyuncak, arabası ve anne-bebek ürünleri
        </p>
      </div>
