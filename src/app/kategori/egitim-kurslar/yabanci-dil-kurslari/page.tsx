...
export default function LanguageCoursesCategoryPage() {
  const [selectedLanguage, setSelectedLanguage] = useState<string | null>(null)
  const [priceRange, setPriceRange] = useState<string | null>(null)
  const [location, setLocation] = useState<string | null>(null)
  const [courseType, setCourseType] = useState<string | null>(null)

  // Yabancı dil kursu ilanlarını filtrele
  const languageCourseListings = listings.filter(listing => 
    listing.category === 'egitim-kurslar' && 
    listing.subcategory === 'yabanci-dil-kurslari'
  )

  // Dilleri topla
  const languages = Array.from(new Set(languageCourseListings.map(listing => listing.brand)))

  // Filtreleme fonksiyonu
  const filteredListings = languageCourseListings.filter(listing => {
    if (selectedLanguage && listing.brand !== selectedLanguage) return false
      ...
      <div className="flex flex-col md:flex-row gap-8">
        ...
        <div className="w-full md:w-64 flex-shrink-0">
          <div className="bg-white rounded-lg shadow-sm p-4">
            ...
            {/* Dil Filtresi */}
            <div className="mb-6">
              <h3 className="font-medium mb-2">Dil</h3>
