...
import { FaCamera, FaVideo, FaCameraRetro } from 'react-icons/fa'
import { useState } from 'react'
import { listings } from '@/lib/listings'
import { ListingCard } from '@/components/listing-card'

const subcategories = [
  { id: 'foto-makinesi', name: 'Fotoğraf Makinesi', icon: <FaCamera className="inline mr-2 text-blue-500" /> },
  { id: 'video-kamera', name: 'Video Kamera', icon: <FaVideo className="inline mr-2 text-blue-500" /> },
  { id: 'go-pro', name: 'GoPro', icon: <FaCameraRetro className="inline mr-2 text-blue-500" /> },
]

export default function KameraCategoryPage() {
  const [selectedSubcategory, setSelectedSubcategory] = useState<string | null>(null)
  const [priceRange, setPriceRange] = useState<string | null>(null)
  const [condition, setCondition] = useState<string | null>(null)

  // Kamera ilanlarını filtrele
  const cameraListings = listings.filter(listing => 
    listing.category === 'elektronik' && 
    listing.subcategory === 'kamera'
  )

  // Filtreleme fonksiyonu
  const filteredListings = cameraListings.filter(listing => {
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        ...
        <p className="text-gray-600 mt-2">
          Fotoğraf makineleri, video kameralar ve aksiyon kameraları
        </p>
      </div>
