...
const subcategories = [
  { id: 'kablolu', name: 'Kablolu Kulaklık', icon: <FaHeadphones className="inline mr-2 text-blue-500" /> },
  { id: 'kablosuz', name: 'Kablosuz Kulaklık', icon: <FaHeadphones className="inline mr-2 text-green-500" /> },
  { id: 'gaming', name: 'Gaming Kulaklık', icon: <FaHeadphones className="inline mr-2 text-red-500" /> },
]

export default function KulaklikCategoryPage() {
  const [selectedSubcategory, setSelectedSubcategory] = useState<string | null>(null)
  const [priceRange, setPriceRange] = useState<string | null>(null)
  const [condition, setCondition] = useState<string | null>(null)

  // Kulaklık ilanlarını filtrele
  const kulaklikListings = listings.filter(listing => 
    listing.category === 'elektronik' && 
    listing.subcategory === 'kulaklik'
  )
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900">Kulaklık</h1>
        <p className="text-gray-600 mt-2">
          Kablolu, kablosuz ve gaming kulaklıklar
        </p>
      </div>
