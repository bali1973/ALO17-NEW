...
const subcategories = [
  { id: 'mobilya', name: 'Bahçe Mobilyası', icon: <FaTree className="inline mr-2 text-green-500" /> },
  { id: 'bitki', name: 'Bitki', icon: <FaLeaf className="inline mr-2 text-green-500" /> },
  { id: 'tohum', name: 'Tohum', icon: <FaSeedling className="inline mr-2 text-green-500" /> },
]

export default function BahceCategoryPage() {
  const [selectedSubcategory, setSelectedSubcategory] = useState<string | null>(null)
  const [priceRange, setPriceRange] = useState<string | null>(null)
  const [condition, setCondition] = useState<string | null>(null)

  // Bahçe ilanlarını filtrele
  const bahceListings = listings.filter(listing => 
    listing.category === 'ev-bahce' && 
    listing.subcategory === 'bahce'
  )

  // Filtreleme fonksiyonu
  const filteredListings = bahceListings.filter(listing => {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900">Bahçe</h1>
        <p className="text-gray-600 mt-2">
          Bahçe mobilyası, bitki ve tohum ürünleri
        </p>
      </div>
