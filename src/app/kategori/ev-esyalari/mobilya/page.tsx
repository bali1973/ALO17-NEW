...
export default function FurnitureCategoryPage() {
  ...
  const furnitureListings = listings.filter(listing => 
    listing.category === 'ev-esyalari' && 
    listing.subcategory === 'mobilya'
  )

  // Markaları topla
  const brands = Array.from(new Set(furnitureListings.map(listing => listing.brand)))

  // Filtreleme fonksiyonu
  const filteredListings = furnitureListings.filter(listing => {
