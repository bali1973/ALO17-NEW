...
export default function EvEsyalariPage() {
  ...
  const mainCategory = categories.find(cat => cat.slug === 'ev-esyalari');
  ...
  useEffect(() => {
    fetch('/api/listings')
      .then(res => res.json())
      .then(data => {
        setListings(data.filter((listing: Listing) => listing.category === 'ev-esyalari'));
      });
  }, []);
  ...
  return (
    <CategoryLayout
      ...
      activeSlug=""
      categoryBasePath="kategori/ev-esyalari"
      city={city}
    ...
    >
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Ev Eşyaları</h1>
        <p className="text-gray-600 mt-2">
          Beyaz eşya, mobilya ve ev eşyaları
        </p>
      </div>
