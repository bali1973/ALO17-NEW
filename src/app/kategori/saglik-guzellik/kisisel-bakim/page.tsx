...
import { FaHeartbeat, FaShower, FaCut, FaSpa } from 'react-icons/fa'
import { useState } from 'react'
import { listings } from '@/lib/listings'
import { ListingCard } from '@/components/listing-card'

const subcategories = [
  { id: 'cilt-bakimi', name: 'Cilt Bakımı', icon: <FaHeartbeat className="inline mr-2 text-red-500" /> },
  { id: 'sac-bakimi', name: 'Saç Bakımı', icon: <FaShower className="inline mr-2 text-blue-500" /> },
  { id: 'makyaj', name: 'Makyaj', icon: <FaCut className="inline mr-2 text-purple-500" /> },
  { id: 'parfum', name: 'Parfüm', icon: <FaSpa className="inline mr-2 text-green-500" /> },
]

export default function KisiselBakimPage() {
  const [selectedSubcategory, setSelectedSubcategory] = useState<string | null>(null)
  const [priceRange, setPriceRange] = useState<string | null>(null)
  const [condition, setCondition] = useState<string | null>(null)

  // Kişisel Bakım ilanlarını filtrele
  const kisiselBakimListings = listings.filter(listing => 
    listing.category === 'saglik-guzellik' && 
    listing.subcategory === 'kisisel-bakim'
  )

  // Filtreleme fonksiyonu
  const filteredListings = kisiselBakimListings.filter(listing => {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900">Kişisel Bakım</h1>
        <p className="text-gray-600 mt-2">
          Cilt bakımı, saç bakımı, makyaj ve parfüm ürünleri
        </p>
      </div>
