...
export default function SpaCenterCategoryPage() {
  const [selectedType, setSelectedType] = useState<string | null>(null)
  const [priceRange, setPriceRange] = useState<string | null>(null)
  const [location, setLocation] = useState<string | null>(null)
  const [services, setServices] = useState<string[]>([])

  // Spa merkezi ilanlarını filtrele
  const spaListings = listings.filter(listing => 
    listing.category === 'saglik-guzellik' && 
    listing.subcategory === 'spa-merkezi'
  )

  // Merkez tiplerini topla
  const types = Array.from(new Set(spaListings.map(listing => listing.type)))

  // Filtreleme fonksiyonu
  const filteredListings = spaListings.filter(listing => {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900">Spa Merkezi</h1>
        <p className="text-gray-600 mt-2">
          Masaj, cilt bakımı ve wellness hizmetleri.
        </p>
      </div>
          ...
          {filteredListings.length === 0 && (
            <div className="text-center py-12">
              ...
              <p className="text-gray-600">
                Seçtiğiniz kriterlere uygun spa merkezi bulunamadı. Lütfen filtreleri değiştirerek tekrar deneyin.
              </p>
            </div>
          )}
        </div>
