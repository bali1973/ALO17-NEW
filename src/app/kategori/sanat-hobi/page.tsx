...
export default function SanatHobiPage() {
  ...
  const mainCategory = categories.find(cat => cat.slug === 'sanat-hobi');
  ...
  useEffect(() => {
    fetch('/api/listings')
      .then(res => res.json())
      .then(data => {
        setListings(data.filter((listing: Listing) => listing.category === 'sanat-hobi'));
      });
  }, []);
  ...
  return (
    <CategoryLayout
      ...
      activeSlug=""
      categoryBasePath="kategori/sanat-hobi"
      city={city}
    ...
    >
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Sanat & Hobi</h1>
        <p className="text-gray-600 mt-2">
          Sanat malzemeleri, hobi ürünleri ve yaratıcı projeler
        </p>
      </div>
