...
export default function TurizmGecelemelerPage() {
  ...
  const mainCategory = categories.find(cat => cat.slug === 'turizm-gecelemeler');
  ...
  useEffect(() => {
    fetch('/api/listings')
      .then(res => res.json())
      .then(data => {
        setListings(data.filter((listing: Listing) => listing.category === 'turizm-gecelemeler'));
      });
  }, []);
  ...
  return (
    <CategoryLayout
      ...
      activeSlug=""
      categoryBasePath="kategori/turizm-gecelemeler"
      city={city}
    ...
    >
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Turizm & Gecelemeler</h1>
        <p className="text-gray-600 mt-2">
          Oteller, pansiyonlar, tatil köyleri ve konaklama seçenekleri
        </p>
      </div>
